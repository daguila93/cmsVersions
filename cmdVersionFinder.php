...
//Testa se o site é um WP
    if (file_exists($wordpressFilePath)) {
        $wordpressCommand = shell_exec("grep '\$wp_version =' " . $wordpressFilePath);
        $source .= "$sitesArray[$i],WORDPRESS," . trim(str_replace('$wp_version = ', "", $wordpressCommand), "");
    }

//Testa se o site é um Joomla
    if (file_exists($joomlaFilePath)) {
...
//Testa se o site é um Drupal
    if (file_exists($drupalFilePath)) {
...
//Testa se o site é um OJS
    if (file_exists($ojsFilePath)) {
